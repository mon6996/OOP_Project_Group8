...
//        }
    ...
    public void relocate()
    {
        ...
        {
            --budget;
            Region cityCenter = Upbeat.getRegion(cityCenter_m, cityCenter_n);
            Region cur = Upbeat.getRegion(cityCrew_m, cityCrew_n);
            int[] next;
            if (cur.getOwner().equals(this))
            {
                ...
                while ((cur.getRow() != cityCenter.getRow() && cur.getCol() != cityCenter.getCol()) || cur.getRow() != cityCenter.getRow() || cur.getCol() != cityCenter.getCol())
                {
                    if(cityCenter.getCol() < cur.getCol())
                    {
                        ...
                        {
                            cityCenter = Upbeat.getRegion(cur.getRow(), cur.getCol());
                        ...
                        else if(cityCenter.getRow() < cur.getRow())
                        {
                            ...
                            cityCenter = Upbeat.getRegion(next[0], next[1]);
                        }
                        else
                        {
                ...
                {
                    budget -= cost;
                    Upbeat.getRegion(cityCenter_m, cityCenter_n).changeCityCenter();
    ...
    public void move(long dir)
    {
        ...
        Player owner = Upbeat.getRegion(currow, curcol).getOwner();
    ...
    public void shoot(long dir, long cost)
    {
        if(budget - cost > 0)
        {
            ...
            Region opponent = Upbeat.getRegion(opRow, opCol);
    ...
    public long opponent()
    {
        ...
        while (opPosition[0] != null && opPosition[1] != null && opPosition[2] != null && opPosition[3] != null && opPosition[4] != null && opPosition[5] != null)
        {
            for (int i = 0; i < 6; i++)
            {
                ...
                Region opponent = Upbeat.getRegion(next[0], next[1]);
    ...
    public long nearby(long dir)
    {
        ...
        {
            dist++;
            Region opponent = Upbeat.getRegion(opPosition[0], opPosition[1]);
